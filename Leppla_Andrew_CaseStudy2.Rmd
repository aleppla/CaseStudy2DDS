...
The Attrition model correctly predicts if an employee will leave about 80% of the time, and correctly predicts if an employee will stay about 65% of the time.
...
Categorical variables were explored for both Naive Bayes and Regression models.  

Continuous variables were also explored for k Nearest Neighbors (kNN) and Regression models. 
...
CaseStudy2 %>% ggplot(aes(Age,fill=Attrition)) + geom_histogram(position="dodge")
CaseStudy2 %>% ggplot(aes(Age,fill=Attrition)) + geom_histogram(position="fill") + ylab("Proportion")
...
par(mfrow=c(3,3))
plot(Attrition~BusinessTravel,col=rainbow(11),data=CaseStudy2) 
#Slightly increasing trend

plot(Attrition~Department,col=rainbow(11),data=CaseStudy2) 
#Small effect

plot(Attrition~Educ,col=rainbow(11),data=CaseStudy2) 
# Slightly decreasing effect

plot(Attrition~EnvSat,col=rainbow(6),data=CaseStudy2)  
# EnvSat=1 has higher Attrition vs. 2-4

plot(Attrition~Gender,col=rainbow(6),data=CaseStudy2)
# No effect by itself

############################################################
plot(Attrition~MaritalStatus,col=rainbow(2),data=CaseStudy2)
...
K Nearest Neighbors (kNN) was unsuccessful at reliably predicting Attrition with continuous variables.  kNN predictive models were not able to consistently achieve 60% sensitivity and 60% specificity in both the training and test sets.  
...
Using categorical variables, a Naive Bayes model was able to predict Attrition with about 65% Sensitivity (correct No's) and 80% Specificity (correct Yes's).  This was achieved by increasing the prediction threshold from 0.5 to 0.9 to more accurately predict the Yes's for Attrition.    
...
NB1 = naiveBayes(Attrition~OverTime+JobRole+MaritalStatus+JobSat+EnvSat,data = train_CS2) 
...
# Change the prediction threshold from 0.5
# To increase Specificity
thresh <- 0.89
out <- factor(ifelse(pred1_NB_train[,1] > thresh, "No","Yes"), levels = c("No","Yes"))
confusionMatrix(out,train_CS2$Attrition)
...
# To increase Specificity
thresh <- 0.89
out <- factor(ifelse(pred1_NB_test[,1] > thresh, "No","Yes"), levels = c("No","Yes"))
confusionMatrix(out,test_CS2$Attrition)

# Full Data Set

NB_full = naiveBayes(Attrition~OverTime+JobRole+MaritalStatus+JobSat+EnvSat,data = CaseStudy2) 
...
# Change the prediction threshold from 0.5
# To maximize Pred Value for Attrition=Yes
thresh <- 0.89
out <- factor(ifelse(pred1_NB_full[,1] > thresh, "No","Yes"), levels = c("No","Yes"))
confusionMatrix(out,CaseStudy2$Attrition)
```


## Random repeat training/test splits for Naive Bayes

Naive Bayes model is robust and replicable with 30 different random training/test splits.
...
NB100 = naiveBayes(Attrition~OverTime+JobRole+MaritalStatus+JobSat+EnvSat+JobLvl,data = train_CS) 
#Top Factors: JobRole, JobInvolvement, OverTime
#Improves one metric vs. another: JobLvl, JobRole, JobInv 
#Minimal/No Improvement: EducationField, EnvSat
...
out <- factor(ifelse(pred1_NB100_train[,1] > thresh, "No","Yes"), levels = c("No","Yes"))
...
out <- factor(ifelse(pred1_NB100_test[,1] > thresh, "No","Yes"), levels = c("No","Yes"))
...
plot(weighted.residuals(qm_wtd)~qm1_wtd$fitted.values,xlab="Fitted Values",ylab="Weighted Residuals", main="Weighted Residuals vs. Fitted")
...
Finally, an LDA model was tested to model Attrition.  Monthly Income alone was a reasonably good predictor with 60% Sensitivity and Specificity.  A more complex LDA model with most of the numeric predictors improved to 70% Sensitivity and Specificity, or 60% Sensitivity (correct No's) and 80% Specificity (correct Yes's).  This model could likely be reduced/improved by removing insignificant terms.          
...
my_lda2a <- lda(Attrition~., data = CS_dump,CV=T,prior=c(0.4,0.6)) 
CM_2a = confusionMatrix(table(my_lda2a$class,CaseStudy2$Attrition)) 
CM_2a
# 82% sensitivity and 61% specificity
